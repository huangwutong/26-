...
function deleteCompany()
{
	...
	if(ids != "")
	{
		form1.action = "fdelectcompany?cleintids="+ids;
		form1.method="post";
...
function tocCompany()
{
	...
	if(ids != "")
	{
		form1.action = "fdelectcompany?cleintids="+ids+"&toc=1";
		form1.method="post";
		document.form1.submit();
	}
}
function attornCleint()
{
	...
	if(ids != "")
	{
	 	form1.action = "turn?type=fa&cleintids="+ids;
		form1.method="post"; 
		document.form1.submit();
	}
}
function attornall(perid)
{
	form1.action = "turn?type=all&personnel="+perid;
	form1.method="post"; 
	document.form1.submit();
}
var v_request;
	...
	行业
		...
		所在库<select name="state" id="state">
		  ...
		  客户类型
 			...
 			<a href="javascript:document.form1.submit();" class="pagenum">搜索</a>
 			<c:if test="${sessionScope.pos==null}">
 			<br/>
 			选择：${requestScope.xls }&nbsp;<a href="javascript:formObj.chooseAll(true);" class="pagenum">全选</a>&nbsp;&nbsp;
    		<a href="javascript:formObj.chooseAll(false);" class="pagenum">取消</a>&nbsp;&nbsp;
 			<a href="javascript:attornCleint();" class="pagenum">转让客户</a>
 			<c:if test="${sessionScope.perid!=null}">
 				<a href="javascript:attornall(${sessionScope.perid});" class="pagenum">转让所有客户</a>
 			</c:if>
 			<a href="javascript:tocCompany();" class="pagenum">转入C库</a>
 			 <a href="javascript:deleteCompany();" class="pagenum">删除客户</a></c:if>&nbsp;&nbsp;&nbsp;&nbsp;<br/>${requestScope.csize }
    </td>
  </tr>
  <tr>
  ...
  <tr>
    <td class="contentList"><table id="tablecompany" width="100%" border="0" cellpadding="8" cellspacing="1" class="contentListTable">
      ...
      <tr class="contentListTd">
        ...
        <td><a href="finfo?companyid=${company.companyid}&f=1"><img src="images/btn_01.gif" width="15" height="15" border="0" /></a></td>
      </tr>
	</c:forEach>
	<tr class="contentListTd">
