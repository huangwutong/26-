...
function getid()
{
	...
	form1.action = "turn?type=bdc&cleintids="+ids;
	if(ids != "")
	{
...
function attornCleint()
{
	...
	form1.action = "turn?type=ba&cleintids="+ids; 
	if(ids != "")
	{
		document.form1.submit();
	}
...
<table width="100%" border="0" cellpadding="8" cellspacing="1" class="contentArea">
  <tr>
    ...
    <a href="javascript:attornCleint();" class="pagenum">转让客户</a>&nbsp;&nbsp;&nbsp;&nbsp;${requestScope.csize }</td>
  </tr>
  <tr>
...
<c:if test="${company.adddate == company.indate }">
      ...
      <td>&nbsp;<c:if test="${(100-(sessionScope.date.time-company.indate.time)/1000/60/60/24)<10}">
      ${100-(sessionScope.date.time-company.indate.time)/1000/60/60/24 }</c:if></td>
      <td>&nbsp;${company.visitcount }</td>
      <td>&nbsp;${company.type }</td>
      <td>&nbsp;${company.lastvisitdate }</td>
      <td>&nbsp;${company.industry.industry }</td>
      <td><a href="infoB?companyid=${company.companyid}&size=${requestScope.size }"><img src="images/btn_01.gif" width="15" height="15" border="0" /></a></td>
      </tr>
</c:if>
<c:if test="${company.adddate != company.indate }">
      <tr class="contentListTd2">
      <td>&nbsp;<c:if test="${company.visitcount>0 }"><input type="checkbox" name="idnum" value="${company.companyid}"/></c:if></td>        
      <td>&nbsp;${company.nameparticular } </td>
      <td>&nbsp;${company.indate } </td>
      <td>&nbsp;<c:if test="${(100-(sessionScope.date.time-company.indate.time)/1000/60/60/24)<10}">
...
<div id="helpContent">
  ...
  <br/>同时提醒您，如果你要对某个客户进行修改操作，请进入客户详细资料内操作。
  <br/>如果您要进行转让客户或放弃客户等操作前，请先把您要操作的客户选中，再行操作，顺便提醒您，这一类操作可以多个客户同时进行。
