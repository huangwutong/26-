...
//		Personnel user = (Personnel)request.getSession().getAttribute("user");
		RequestDispatcher rd = request.getRequestDispatcher("crm/c/list.jsp");
		if (cm.getConnection())
		{
...
//				companies = temp;
				...
				if (companies.size() != 0)
				{
					...
					if (page != 1)
					{
						htmlPage = htmlPage + "&nbsp;<a href='selectC?page=1' class='pagenum'>首页</a>";
						htmlPage = htmlPage + "&nbsp;<a href='selectC?page=" + (page - 1) + "' class='pagenum'>上一页</a>";
					}
					int i = page / 10;
					if (i == 0)
					{
					...
					for (; i <= j; i++)
					{
						if (i != page)
						{
							htmlPage = htmlPage + "&nbsp;<a href='selectC?page=" + i + "' class='pagenum'>" + i + "</a>";
						}
						else
						{
							htmlPage = htmlPage + "&nbsp;" + i;
					...
					if (page != pageCount)
					{
						htmlPage = htmlPage + "&nbsp;<a href='selectC?page=" + (page + 1) + "' class='pagenum'>下一页</a>";
						htmlPage = htmlPage + "&nbsp;<a href='selectC?page=" + (pageCount) + "' class='pagenum'>尾页</a>";
					}
