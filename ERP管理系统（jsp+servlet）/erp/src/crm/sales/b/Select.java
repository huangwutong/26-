...
public class Select extends HttpServlet
{
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		RequestDispatcher rd = request.getRequestDispatcher("crm/b/list.jsp");
		if (cm.getConnection())
		{
			List<Company> companies = null;
			String sql = "select * from company where companystate = 'B' and personnelid = " + user.getPersonnelid()+ " order by adddate desc";
			...
			if (p)
			{
				...
				if (companies.size() != 0)
				{
					...
					if (page != 1)
					{
						htmlPage = htmlPage + "&nbsp;<a href='select?page=1' class='pagenum'>首页</a>";
						htmlPage = htmlPage + "&nbsp;<a href='select?page=" + (page - 1) + "' class='pagenum'>上一页</a>";
					}
					int i = page / 10;
					if (i == 0)
					{
					...
					for (; i <= j; i++)
					{
						if (i != page)
						{
							htmlPage = htmlPage + "&nbsp;<a href='select?page=" + i + "' class='pagenum'>" + i + "</a>";
						}
						else
						{
							htmlPage = htmlPage + "&nbsp;" + i;
					...
					if (page != pageCount)
					{
						htmlPage = htmlPage + "&nbsp;<a href='select?page=" + (page + 1) + "' class='pagenum'>下一页</a>";
						htmlPage = htmlPage + "&nbsp;<a href='select?page=" + (pageCount) + "' class='pagenum'>尾页</a>";
					}
