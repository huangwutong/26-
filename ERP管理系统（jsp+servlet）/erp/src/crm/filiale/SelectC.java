...
public class SelectC extends HttpServlet
{
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		RequestDispatcher rd = request.getRequestDispatcher("crm/filiale/clist.jsp");
		if (cm.getConnection())
		{
			List<Company> companies = null;

			String sql = "select * from company where companystate = 'C' order by adddate desc";
			...
			if (p)
			{
				...
				if (companies.size() != 0)
				{
					...
					if (page != 1)
					{
						htmlPage = htmlPage + "&nbsp;<a href='fcList?page=1' class='pagenum'>首页</a>";
						htmlPage = htmlPage + "&nbsp;<a href='fcList?page=" + (page - 1) + "' class='pagenum'>上一页</a>";
					}
					int i = page / 10;
					if (i == 0)
					{
					...
					for (; i <= j; i++)
					{
						if (i != page)
						{
							htmlPage = htmlPage + "&nbsp;<a href='fcList?page=" + i + "' class='pagenum'>" + i + "</a>";
						}
						else
						{
					...
					if (page != pageCount)
					{
						htmlPage = htmlPage + "&nbsp;<a href='fcList?page=" + (page + 1) + "' class='pagenum'>下一页</a>";
						htmlPage = htmlPage + "&nbsp;<a href='fcList?page=" + (pageCount) + "' class='pagenum'>尾页</a>";
					}
