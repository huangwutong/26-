...
<%
	...
	try{
		...
		int parentID=ParamUtil.getInt(request,"parentID");
	...
	stats=""+theForum.getForumType()+"发言";
...
%>
		...
		<form action="SaveReAnnounce.jsp?forumID=<%=forumID%>" method="POST" onSubmit="return check(this)" name="frmAnnounce">
		        ...
		        <tr bgcolor="<%=tableBodyColor%>">
		          <td width="14%"><font color="<%=tableContentColor%>"><b>主题标题</b>
		              ...
		              <OPTION value=[分享]>[分享]</OPTION></SELECT>
				            ...
				            <td width="86%"><font color="<%=tableContentColor%>"><input name="subject" size=60 maxlength=80 class=FormClass>&nbsp;&nbsp;<font color="red"><strong>*</strong></font>不得超过 50 个汉字
			 </td>
		        </tr>
		        <tr bgcolor="<%=tableBodyColor%>">
		...
		<%
			String tempString="";
		if(ParamUtil.getString(request,"reply","").equals("true")){
			ForumMSG forumMSG=new ForumMSG(parentID);
			
			tempString="[quote][b]以下是引用[i]"+forumMSG.getUserName()+"在"+forumMSG.getDateAndTime()+"[/i]的发言：[/b]\n";
			tempString+=forumMSG.getNoFilterBody()+"\n";
			tempString+="[/quote]\n";
		}
		%>
		<textarea class="smallarea" cols="80" name="content" rows="12" wrap="VIRTUAL" title="可以使用Ctrl+Enter直接提交贴子" class=FormClass onkeydown=ctlent()><%=tempString%></textarea>
		          </td>
		        </tr>
				<tr>
		                <td bgcolor="<%=tableBodyColor%>" valign=top colspan=2><font color="<%=tableContentColor%>"><b>点击表情图即可在帖子中加入相应的表情</B><br>&nbsp;
		...
		<%}%>
		                ...
		                <tr bgcolor="<%=tableBodyColor%>">
		                <td valign=top><font color="<%=tableContentColor%>"><b>选项</b><p><a href="javascript:openScript('help.jsp',500,400)"><img src="pic/help_b.gif" border=0></a></font></td>
		                <td valign=middle><font color="<%=tableContentColor%>"><input type=checkbox name="signFlag" value="1" checked>是否显示您的签名？<br>
