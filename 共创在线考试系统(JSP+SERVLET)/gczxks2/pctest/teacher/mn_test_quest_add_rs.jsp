...
temp=msgsb.toString();
if(debug){
    out.print("<br><br><br>New Information:<br>");
    out.print(temp);
...
sql="SELECT  `test`.`paper`,`test`.`testnum`,`test`.`mark` "+
	...
	sql="update mn_test set mntotalmark=mntotalmark+"+mark+
		" where id="+mntestID;
	stmt=con.createStatement();
	stmt.executeUpdate(sql);
	stmt.close();
	con.commit();
	if(debug)
		out.print("<br>update mntest sql="+sql);
	if(group.equals("single") && !debug){//提交单个试题
