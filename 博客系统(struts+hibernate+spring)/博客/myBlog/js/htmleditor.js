...
this.width =  "auto";
this.height = "auto";
this.bodyStyle = 'background-color: #FFFFFF; font-family: "Verdana"; font-size: 12px;';
this.imgURL = _editor_url + 'htmlarea_images/';
this.debug  = 0;
...
//  ['strikethrough','subscript','superscript','separator'],
    ...
    ['HorizontalRule','Createlink','InsertImage','InsertFlash','insertMulti','InsertTable','htmlmode','separator'],
...
this.btnList = {
    ...
    "subscript":      ['SubScript',            '写在下方',           'editor_action(this.id)',  'ed_format_sub.gif'],
    "superscript":    ['SuperScript',          '写在上方',        	 'editor_action(this.id)',  'ed_format_sup.gif'],
    ...
    "outdent":        ['Outdent',              '减少缩进',    		 'editor_action(this.id)',  'ed_indent_less.gif'],
    "indent":         ['Indent',               '增加缩进',    		 'editor_action(this.id)',  'ed_indent_more.gif'],
    ...
    "createlink":     ['CreateLink',           '插入链接',    		 'editor_action(this.id)',  'ed_link.gif'],
    "insertimage":    ['InsertImage',          '插入图像',       	 'editor_action(this.id)',  'ed_image.gif'],
    "insertflash":    ['InsertFlash',          '插入Flash动画',    	 'editor_action(this.id)',  'ed_flash.gif'],
    "insertmulti":    ['InsertMulti',          '插入多媒体文件',    	 'editor_action(this.id)',  'ed_multi.gif'],
    "inserttable":    ['InsertTable',          '插入表格',       	 'editor_action(this.id)',  'insert_table.gif'],
    "htmlmode":       ['HtmlMode',             '查看HTML源码',   	 'editor_setmode(\''+objname+'\')', 'ed_html.gif'],
    "popupeditor":    ['popupeditor',          '加大编辑器',     	 'editor_action(this.id)',  'fullscreen_maximize.gif'],
    "about":          ['about',                '编辑器信息',  		 'editor_about(\''+objname+'\')',  'ed_about.gif'],
...
function editor_action(button_id) {
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    if (cmdID.toLowerCase() == 'createlink'){
      showModalDialog(_editor_url + "popups/insert_link.jsp?"+objname, window, "resizable: no; help: no; status: no; scroll: no; ");
    }
    ...
    // insert flash
	  ...
	  //重新定义插入Flash动画的功能
      showModalDialog(_editor_url + "popups/insert_flash.html", window, "resizable: no; help: no; status: no; scroll: no; ");
    }

    // insert multi 插入多媒体文件 by Poorfish
    else if (cmdID.toLowerCase() == 'insertmulti'){
      showModalDialog(_editor_url + "popups/insert_multi.jsp?"+objname,
                                 window,
                                 "resizable: yes; help: no; status: no; scroll: no; ");
    }
    ...
    // insert table
      ...
      showModalDialog(_editor_url + "popups/insert_table.jsp?"+objname,
                                 window,
                                 "resizable: yes; help: no; status: no; scroll: no; ");
    }
